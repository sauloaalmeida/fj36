...
public class ConsumidorServicoCorreios {
	...
	public BigDecimal calculaFrete(String cepDestino) {
		...
		CResultado resultado = servico.calcPrecoPrazo(semCodigoEmpresa, semSenhaEmpresa, codigoSedex, cepDestino, cepDestino, peso3kg, formatoEncomendaCaixa, comprimento20cm, altura10cm, largura15cm, diametro10cm, semEntregueEmMaos, semValorDeclarado, semAvisoRecebimento);
		
		List<CServico> servicoPesquisados =  resultado.getServicos().getCServico();
		valorFrete = servicoPesquisados.get(0).getValor();
